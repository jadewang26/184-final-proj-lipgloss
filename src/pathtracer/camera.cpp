...
Ray Camera::generate_ray(double x, double y) const {
  ...
  double normalized_x = 2.0 * (x - 0.5);
  double normalized_y = 2.0 * (y - 0.5);
  
  // scale by field of view
  double sample_x = normalized_x * tan(radians(hFov) / 2);
  double sample_y = normalized_y * tan(radians(vFov) / 2);
  double sample_z = -1.0;  
  Vector3D ray_dir(sample_x, sample_y, sample_z);
  ray_dir.normalize();
  
  // create ray
  Ray output_ray(pos, c2w * ray_dir);
  output_ray.min_t = nClip;
  output_ray.max_t = fClip;
  return output_ray;
